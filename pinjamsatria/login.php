...
<head>
    ...
    <title>MUHAMMAD SATRIA - XIII SIJA 2</title>
...
<body>
    ...
    if (isset($_POST['login'])) {
        ...
        if ($hitung > 0) {
            ...
            $iduser = $ambildatarole['id_user'];
            $namauser = $ambildatarole['nama'];
            if ($role == 'administrator') {
                $_SESSION['iduser'] = $iduser;
                $_SESSION['nama_user'] = $namauser;
                $_SESSION['role'] = 'administrator';
                header('location:./admin/index.php?url=product');
            } elseif ($role == 'peminjam') {
                $_SESSION['iduser'] = $iduser;
                $_SESSION['nama_user'] = $namauser;
                $_SESSION['role'] = 'peminjam';
                header('location:./peminjam/index.php?url=');
            } elseif ($role == 'manajemen') {
                $_SESSION['iduser'] = $iduser;
                $_SESSION['nama_user'] = $namauser;
                $_SESSION['role'] = 'manajemen';
                header('location:./manajemen/index.php?url=');
            }
        }
    }
